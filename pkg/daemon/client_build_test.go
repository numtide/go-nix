...
func TestBuildPathsWithResults(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		rq := require.New(t)
...
func TestBuildDerivation(t *testing.T) {
	t.Run("NilDerivation", func(t *testing.T) {
	...
	// connects at proto 1.27 and calls BuildDerivation. At proto 1.27 the
	// BuildResult wire format omits timing fields (proto < 1.29), CPU times
	// (proto < 1.37), and builtOutputs (proto < 1.28). The mock sends only
	// status + errorMsg.
	t.Run("Proto127", func(t *testing.T) {
		rq := require.New(t)

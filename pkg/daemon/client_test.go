...
// ---------- Version-guard operation tests through mock daemon ----------
...
// TestClientQueryPathInfoProto121 connects at proto 1.21 (MinProtocolVersion)
// and calls QueryPathInfo. At proto 1.21 (which is >= ProtoVersionPathInfoMeta
...
// MinProtocolVersion (1.21), and the client handshake explicitly rejects
// versions below MinProtocolVersion. Since ProtoVersionPathInfoMeta (1.16)
// < MinProtocolVersion (1.21), the "no meta" code path cannot be exercised
...
func TestClientQueryPathInfoProto121(t *testing.T) {
	mock, clientConn := newMockDaemonWithVersion(t, daemon.ProtoVersion(1, 21))
	defer mock.conn.Close()
	...
	go func() {
		...
		// Proto 1.21 >= 1.16, so we DO send ultimate/sigs/ca
	...
	assert.Equal(t, daemon.ProtoVersion(1, 21), client.Info().Version)
	...
	// At proto 1.21 (>= 1.16), ultimate/sigs/ca ARE included
	assert.False(t, info.Ultimate)
	assert.Empty(t, info.Sigs)
	assert.Equal(t, "", info.CA)
}

// TestClientQueryValidPathsPreSubstituteOk connects at proto 1.21
...
// TestClientSetOptionsProto121 connects at proto 1.21 (MinProtocolVersion,
...
func TestClientSetOptionsProto121(t *testing.T) {
	mock, clientConn := newMockDaemonWithVersion(t, daemon.ProtoVersion(1, 21))
	defer mock.conn.Close()

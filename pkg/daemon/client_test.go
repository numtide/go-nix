...
func TestClient(t *testing.T) {
	...
	// exercises multiple different operations on the same mock connection to
	// verify the client properly resets the connection state between operations.
	...
	// verifies that the connection remains usable after the daemon returns an
	// error for one operation.
	t.Run("OperationAfterError", func(t *testing.T) {
		rq := require.New(t)
	...
	t.Run("DaemonErrorWithTraces", func(t *testing.T) {
		rq := require.New(t)
	...
	// verifies that cancelling a context unblocks a pending operation and
	// that the client remains usable for subsequent ops.

...
func newMockDaemonWithVersion(t *testing.T, version uint64) *mockDaemon {
	...
	// use a short path for the socket to stay within the 104-byte Unix socket address limit on macOS
	// t.TempDir() paths include the full test name which can exceed this for longer test names
